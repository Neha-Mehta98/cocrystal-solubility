...
//+ENDPLUMEDOC
    ...
    public:
      ...
      double zetal_y, zetau_y, sigmal_y, sigmau_y;
      ...
      ZrowY(const ActionOptions&);    // CONSTRUCTOR
      ~ZrowY();                       // DESTRUCTOR
      // active methods:
    ...
    PLUMED_REGISTER_ACTION(ZrowY,"ZROWY")

    void ZrowY::registerKeywords( Keywords& keys ){
    
      Colvar::registerKeywords(keys);
      keys.add("atoms", "CENTER", "the labels of the atoms acting as center of the molecules");

      keys.add("compulsory", "ZETAL_Y", "lower position of f_y_i");
      keys.add("compulsory", "ZETAU_Y", "upper position of f_y_i");
      keys.add("compulsory", "SIGMAL_Y", "steepness of f_y_i on lower side");
      keys.add("compulsory", "SIGMAU_Y", "steepness of f_y_i on upper side");
    ...
    ZrowY::ZrowY(const ActionOptions&ao):
      PLUMED_COLVAR_INIT(ao)
    {

      parseAtomList("CENTER", center);

      parse("ZETAL_Y", zetal_y);
      parse("ZETAU_Y", zetau_y);
      parse("SIGMAL_Y", sigmal_y);
      parse("SIGMAU_Y", sigmau_y);
...
void ZrowY::kernel_lu(double q_i, double zetal, double zetau, double sigmal, double sigmau) {
...
void ZrowY::layerindices(vector<Vector> &pos, vector<int> &list) {
  ...
  //for (unsigned int i=rank; i<nmol; i+=stride) {
    ...
    if ( (lb < pos_i[2]) && (pos_i[2] < ub) ) {
      ...
      //if (pos[k][1] < 0) {
      //  pos[k][1] = pos[k][1] + getBox()[1][1];
      //} else if (pos[k][1] > getBox()[1][1]) {
      //  pos[k][1] = pos[k][1] - getBox()[1][1];
      //}
...
void ZrowY::calculate()
{
  ...
  for (unsigned int i=rank; i<ll; i+=stride) {  // SUM OVER MOLECULES
    ...
    kernel_lu(pos[i][1], zetal_y, zetau_y, sigmal_y, sigmau_y);
    f_y_i = f_lu_i;
    df_y_i = df_lu_i;
    ...
    //deriv[index_i][0] += 0;
    deriv[index_i][1] += df_y_i*f_z_i;
    deriv[index_i][2] += f_y_i*df_z_i;
